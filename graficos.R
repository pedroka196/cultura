...
grafico_Valores_Linguagem<-linguagem %>%
  ...
  theme(axis.text.x = element_blank(),legend.text = element_text(size=7), legend.title = element_blank(), axis.title=element_blank())+
  ...
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("R$ %s", format(Valor,big.mark = ".", decimal.mark = ",")),
                                                 y=Valor+ValorSD),size=tamanho, show.legend = F, show.legend = F, box.padding = unit(x = 0, units = "pt"))+
...
grafico_Contagem_Linguagem<-linguagem %>%
  ggplot(aes(x=Linguagem,y=Num,fill=Linguagem))+
  geom_col(position = position_dodge(width = 0)) +
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("%s", format(Num, big.mark = ".", decimal.mark = ",")),
                                 y=Num),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
  theme(axis.text.x = element_blank(),legend.text = element_text(size=7), legend.title = element_blank(), axis.title=element_blank())+
...
grafico_Empregos_Linguagem<-linguagem %>%
  ...
  #### 
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("%s", format(Empregos,big.mark = ".", decimal.mark = ",")),
                                 y=Empregos+EmpregosSD),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
...
grafico_Publico_Linguagem<-linguagem %>%
  ...
  geom_col(position = position_dodge(width = 0)) +
  #ggthemes::theme_hc() +
  theme(axis.text.x = element_blank(),legend.text = element_text(size=5), legend.title = element_blank(), axis.title=element_blank())+
  geom_errorbar(mapping=aes(ymin=Publico-PublicoSD,ymax=Publico+PublicoSD),width=0) +
  #### 
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("%s", format(Publico,big.mark = ".", decimal.mark = ",")),
                                 y=Publico+PublicoSD),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
...
grafico_Valores_Natureza<-natureza %>%
  ...
  #### 
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("R$ %s", format(Valor,big.mark = ".", decimal.mark = ",")),
                                 y=Valor+ValorSD),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
...
grafico_Valores_RA_Proponente<-cidades %>%
  ...
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("R$ %s", format(Valor,big.mark = ".", decimal.mark = ",")),
                                 y=Valor+ValorSD),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
  ggtitle("Valores pagos em reais por RA do proponente em 2015",ylab("Em reais"))+
  coord_flip()
...
grafico_Empregos_Escolaridade<-escolaridade %>%
  ...
  theme(legend.position = "none",legend.text = element_text(size=7), legend.title = element_blank(), axis.title=element_blank())+
  geom_errorbar(mapping=aes(ymin=Empregos-EmpregosSD,ymax=Empregos+EmpregosSD),width=0) +
  #### 
  ggrepel::geom_label_repel(stat="identity", aes(label=sprintf("%s", format(Empregos,big.mark = ".", decimal.mark = ",")),
                                 y=Empregos+EmpregosSD),size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
...
grafico_Valores_RA_Atingida<-cidades_atingidas %>%
  #optei por colocar apenas RAs com mais de um projeto executado, para deixar o gráfico mais limpo
  filter(Num>2) %>%
  ggplot(aes(x=reorder(Cidades,ValoresMedios),y=ValoresMedios,fill=Cidades))+
  #Gráfico de colunas
  geom_col(position = position_dodge(width = 0)) +
  theme(legend.text = element_text(size=7), legend.title = element_blank(),legend.position = "none", 
        axis.title = element_blank())+
  ggrepel::geom_label_repel(stat="identity", 
                            aes(label=sprintf("R$ %s", format(ValoresMedios,big.mark = ".", decimal.mark = ",")),
                                 y=ValoresMedios),
                            size=tamanho, show.legend = F, box.padding = unit(x = 0, units = "pt")) +
  ggtitle("Valores médios pagos em reais por RA atingida em 2015",ylab("Em reais"))+
  coord_flip()
...
graficos_todos<-c("grafico_Valores_Linguagem","grafico_Contagem_Linguagem","grafico_Empregos_Linguagem","grafico_Publico_Linguagem",
                  "grafico_Valores_Natureza","grafico_Contagem_Natureza","grafico_Empregos_Natureza","grafico_Publico_Natureza",
                  "grafico_Valores_Cor","grafico_Contagem_Cor","grafico_Empregos_Cor","grafico_Publico_Cor",
                  "grafico_Valores_Sexo","grafico_Contagem_Sexo","grafico_Empregos_Sexo","grafico_Publico_Sexo",
                  "grafico_Valores_Escolaridade","grafico_Contagem_Escolaridade","grafico_Empregos_Escolaridade","grafico_Publico_Escolaridade",
                  "grafico_Valores_RA_Proponente","grafico_Contagem_RA_Proponente","grafico_Empregos_RA_Proponente","grafico_Publico_RA_Proponente",
                  "grafico_Valores_RA_Atingida","grafico_Contagem_RA_Atingida")
